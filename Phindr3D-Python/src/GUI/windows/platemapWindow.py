...
class platemapWindow(QDialog):
        ...
        if first:
            ...
            if type(dataframe) == dict:  # i.e. if Excel file is passed...
                for i in dataframe.keys():  # each key is a sheet name in the xlsx file.
                    ...
                    metapd[i] = metapd.apply(
                        lambda x: df.iloc[int(x["Row"]) - 1, int(x["Column"]) - 1],
                        axis=1,
                    )
                ...
                df = df.replace(np.nan, "", regex=True)
                tabwidget.setTabPosition(QTabWidget.TabPosition.West)
                metapd[name] = metapd.apply(
                    lambda x: df.iloc[int(x["Row"]), int(x["Column"])], axis=1
                )
            ...
            mainLayout.addLayout(
                tableBox, 0, 0, alignment=QtCore.Qt.AlignmentFlag.AlignTop
            )
            mainLayout.addLayout(
                buttonBox, 0, 1, alignment=QtCore.Qt.AlignmentFlag.AlignBottom
            )
            ...
            metapd = pd.read_csv(
                meta, sep="\t", na_values="        NaN"
            )  # this time the meta is a file path.
            ...
            if dataframe is None:
                print(
                    "Error: PLATEMAP is None. Please load a platemap in the previous window."
                )
                return
            ...
            mainLayout.addLayout(
                selectionButtonsBox, 0, 1, alignment=QtCore.Qt.AlignmentFlag.AlignTop
            )
            mainLayout.addWidget(
                submit_button, 1, 1, alignment=QtCore.Qt.AlignmentFlag.AlignBottom
            )
            self.setLayout(mainLayout)
                ...
                for selection in selections:
                    try:
                        ...
                        if self.selectedDataframe is None:
                            ...
                            self.selectedDataframe = pd.concat(
                                [self.selectedDataframe, rows]
                            )
                    except:
    ...
    def submit(self):
        ...
        cwd = os.getcwd()
        # path = cwd + '/tmp.tsv'
        self.selectedDataframe.to_csv(
            cwd + "/tmp-selectedClassSample.tsv", sep="\t", index=False
        )
        self.close()
