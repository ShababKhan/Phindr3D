...
class resultsWindow(QDialog):
    ...
    def __init__(self, color, metadata, platemap=None):
        ...
        doseResponse.triggered.connect(lambda: self.plotDoseResponse())
        estimate.triggered.connect(
            lambda: (
                Clustering.Clustering().cluster_est(self.filtered_data)
                if len(self.plot_data) > 0
                else errorWindow(
                    "Error Dialog",
                    "Please Select Feature File. No data is currently displayed",
                )
            )
        )
        setnumber.triggered.connect(
            lambda: (
                self.setnumcluster(colordropdown.currentText())
                if len(self.plot_data) > 0
                else errorWindow(
                    "Error Dialog",
                    "Please Select Feature File. No data is currently displayed",
        ...
        rotation_disable.triggered.connect(
            lambda: self.main_plot.axes.disable_mouse_rotation()
        )
        ...
        exportdata.clicked.connect(
            lambda: (
                save_file(self, map_type.currentText())
                if len(self.plot_data) > 0
                else errorWindow(
                    "Error Dialog",
                    "Please Select Feature File. No data is currently displayed",
        ...
        sc_plot = self.main_plot.axes.scatter3D(
            [], [], [], s=self.point_size, alpha=self.alpha, depthshade=False
        )  # , picker=True)
        ...
        def check_projection(dim, plot):
            ...
            if (
                self.feature_file
                and colordropdown.count() > 0
                and len(self.plot_data) > 0
            ):
        ...
        twod.toggled.connect(
            lambda: (
                check_projection("2d", map_type.currentText())
                if twod.isChecked()
                else None
            )
        )
        threed.toggled.connect(
            lambda: (
                check_projection("3d", map_type.currentText())
                if threed.isChecked()
                else None
            )
        )
        ...
        colordropdown.currentIndexChanged.connect(
            lambda: (
                self.data_filt(
                    colordropdown, self.projection, map_type.currentText(), False
                )
        ...
        map_type.currentIndexChanged.connect(
            lambda: (
                self.data_filt(
                    colordropdown, self.projection, map_type.currentText(), True
                )
                if self.feature_file and colordropdown.count() > 0
                else None
            )
    ...
    def loadFeaturefile(self, grouping, plot, new_plot, prevfile=None):
        ...
        if filename != "" or (
            not isinstance(prevfile, type(None)) and os.path.exists(prevfile)
        ):
            try:
    ...
    def color_groupings(self, grouping):
        ...
        feature_data = pd.read_csv(
            self.feature_file[0], sep="\t", na_values="        NaN"
        )
        ...
        win = selectWindow(
            chk_lbl,
            col_lbl,
            "Filter Feature File Groups and Channels",
            "Grouping",
            "Channels",
            grps,
            filt_lbl,
    ...
    def pointSizeFeature(self):
        ...
        feature, ok = QInputDialog.getItem(
            self,
            "Feature Selection",
            "Select a feature to determine point size:",
            columns,
            0,
            False,
        )
    ...
    def updatePointSize(self):
        ...
        if self.pointSize_feature != "Default: Constant Size":
            ...
            feature_values = (
                feature_data[self.pointSize_feature].to_numpy().astype(np.float64)
            )
    ...
    def chooseDataSubset(self):
        ...
        try:
            view = platemapWindow(
                dataframe=self.platemap, meta=self.feature_file[0], first=False
            )
            view.show()
            view.exec()
        except:
    ...
    def data_filt(self, grouping, projection, plot, new_plot):
        ...
        image_feature_data = pd.read_csv(
            self.feature_file[0], sep="\t", na_values="        NaN"
        )
        # Identify columns
        ...
        featurecols = columns[
            columns.map(lambda col: col.startswith("MV") or col.startswith("text_"))
        ]
        ...
        image_feature_data = image_feature_data[
            np.isfinite(image_feature_data[featurecols]).all(1)
        ]
        ...
        if np.array_equal(mind, maxd) == False:
            ...
            mv_cols = featuredf.columns[
                featuredf.columns.map(lambda col: col.startswith("MV"))
            ]
            # select data
            if len(self.filt) == 1:
                if self.filt[0] == "MV":
            ...
            # reset imageIDs
            self.imageIDs.clear()
            self.imageIDs.extend(
                np.array(mdatadf["ImageID"], dtype="object").astype(int)
            )
            # reset labels
    ...
    def classificationRF(self):
        ...
        try:
            ...
            features = mapOnlyDF.columns[
                mapOnlyDF.columns.map(lambda col: col.startswith("MV"))
            ]

            # standardize the features
            mapOnlyDF[features] = (
                mapOnlyDF[features] - mapOnlyDF[features].mean()
            ) / mapOnlyDF[features].std()
            ...
            search = GridSearchCV(
                initialRF, space, scoring="accuracy", cv=cv_inner, refit=True
            )
            # fit the model
            ...
            errorWindow(
                "Error Dialog",
                "Please select Feature file and load platemap in the Main window. No data is currently displayed.",
            )
            pass
    ...
    def plotDoseResponse(self):
        ...
        if filename != "":
            ...
            class_, ok = QInputDialog.getItem(
                self, "Class Selection", "Select a class to plot:", classes, 0, False
            )
            ...
            if ok:
                ...
                ax.xaxis.set_major_locator(
                    matplotlib.ticker.LogLocator(base=10.0, subs=(0.25, 0.5, 1.0))
                )
