...
class segmentationWindow(QDialog):
    ...
    def __init__(self, metadata):
        ...
        def setSegmentationSettings(self):
            ...
            intensity = QLineEdit(
                str(self.segmentation.settings["intensity_threshold"])
            )
            ...
            removeborderbox.setChecked(
                self.segmentation.settings["remove_border_objects"]
            )
            channelSelect = QComboBox()
            channelSelect.addItems(["All Channels", "1", "2", "3"])
            ...
            def updateSettingVals(self):
                ...
                try:
                    self.segmentation.settings["min_area_spheroid"] = float(
                        minarea.text()
                    )
                    self.segmentation.settings["intensity_threshold"] = float(
                        intensity.text()
                    )
                    self.segmentation.settings["radius_spheroid"] = float(radius.text())
                    self.segmentation.settings["smoothin_param"] = float(
                        smoothing.text()
                    )
                    self.segmentation.settings["scale_spheroid"] = float(scale.text())
                    self.segmentation.settings["entropy_threshold"] = float(
                        entropy.text()
                    )
                    self.segmentation.settings["max_img_fraction"] = float(
                        maximage.text()
                    )
                    self.segmentation.settings["remove_border_objects"] = (
                        removeborderbox.isChecked()
                    )
                    self.segmentation.settings["seg_Channel"] = (
                        channelSelect.currentText()
                    )
                    return True
                except ValueError:
            ...
            def loadClicked(self, loadbutton):
                ...
                if os.path.exists(settingfile):
                    try:
                        ...
                        alert = self.buildErrorWindow(
                            "Failed to load segmentation settings file.",
                            QMessageBox.Icon.Critical,
                            "Load error",
                        )
                        alert.exec()
                    ...
                    load_setting_win = self.buildErrorWindow(
                        "Select valid Segmentation Settings File (.json)",
                        QMessageBox.Icon.Critical,
                        "Select valid file",
                    )
        ...
        def loadMetadata(self, loadbutton):
            ...
            filename, dump = QFileDialog.getOpenFileName(
                self, "Select Metadata File", "", "Text files (*.tsv)"
            )
            if os.path.exists(filename):
                try:
                    ...
                    alert = self.buildErrorWindow(
                        "Metadata Extraction Failed: Metadata file does not exist.",
                        QMessageBox.Icon.Critical,
                        "Metadata error",
                    )
                    alert.exec()
                ...
                load_metadata_win = self.buildErrorWindow(
                    "Select Valid Metadata File (.tsv)",
                    QMessageBox.Icon.Critical,
                    "Select valid file",
                )
        ...
        def setOutputPath(self, outputbutton):
            ...
            dirname = QFileDialog.getExistingDirectory(
                self, "Select Segmentation Output Directory"
            )
        ...
        def segmentImages(self):
            ...
            if self.segmentation.outputDir == None:
                ...
                if self.segmentation.outputDir == None:
                    return None
            alert = self.buildErrorWindow(
                "Start Segmentation?", QMessageBox.Icon.Information, "Begin"
            )
            alert.exec()
            ...
            if self.segmentation.segmentationSuccess:
                alert = self.buildErrorWindow(
                    "Segmentation Completed.", QMessageBox.Icon.Information, "Complete"
                )
                alert.exec()
