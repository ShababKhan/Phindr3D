...
class regexWindow(QDialog):
    ...
    def __init__(self):
        ...
        self.samplefilebox.setAlignment(
        ...
        instText = (
            "To extract metadata info from the Sample File Name, "
            ...
            "\nDo not use the following reserved group names: "
            + ", ".join(self.reservedKeys)
        )
        ...
        def addRegexGroup():
            ...
            if sellength == 0:
                errWin = self.buildErrorWindow(
                    "Use the cursor to select a value in the file name to associate with the Group Name.",
            ...
            testVal = any(
                ele == previousUseVal for ele in self.fileToRegexPos[selstart:selend]
            )
            if testVal:
                errWin = self.buildErrorWindow(
                    "You have already defined a group that includes part or all "
            ...
            for cx in range(elAfterLabel):
                addVal = (
                    0
                    if self.fileToRegexPos[selend + cx] == previousUseVal
                    else (lenGroupLabel - sellength)
                )
...
class example_regex(QDialog):
    ...
    def __init__(self):
        ...
        # example image 1
        ex1_text = (
            "Ex 1) If trying to extract ch3 which "
        ...
        # example image 2
        ex2_text = (
            "Ex 2) Don't highlight metadata prefix/separators "
        ...
        incorrect_img.setPixmap(
            QPixmap(os.path.join(path, "regex_incorrect_example.png"))
        )
        # add Widgets/layout
