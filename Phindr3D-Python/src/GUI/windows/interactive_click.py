...
class interactive_points:
    ...
    def __init__(
        self, main_plot, projection, data, labels, feature_file, color, imageID
    ):
    ...
    def buildImageViewer(self, label, cur_label, index, color, feature_file, imageID):
        ...
        slice_btn.toggled.connect(
            lambda: (
                ...
                if pjt_box.findChildren(QRadioButton)[0].isChecked()
                else None
            )
        )
        mit_btn.toggled.connect(
            lambda: (
                ...
                if pjt_box.findChildren(QRadioButton)[1].isChecked()
                else None
            )
        )
        montage_btn.toggled.connect(
            lambda: (
                ...
                if pjt_box.findChildren(QRadioButton)[2].isChecked()
                else None
            )
        )
        ch_colour.clicked.connect(
            lambda: self.color_change(
                (ch_info.text()).count("Channel_"),
                self.color,
                "Custom Colour Picker",
                "Channels",
                [
                    "Channel_" + str(i + 1)
                    for i in range((ch_info.text()).count("Channel_"))
                ],
                adjustbar,
                main_plot,
                label,
    ...
    def read_featurefile(
        ...
        if feature_file:
            ...
            if (
                any("Channel" in ch for ch in cols) == False
                or ("Stack" not in cols)
                or ("ImageID" not in cols)
            ):
                self.error = True
            ...
            file_text = (
                ...
                .iloc[meta_loc + slicescrollbar.value()]
            )
            if len(file_text) > 20:
            ...
            if PIL.Image.open(
                ...
                .str.replace(r"\\", "/", regex=True)
                .iloc[meta_loc + slicescrollbar.value()]
            ).mode in ["CMYK", "HSV", "LAB", "RGB", "RGBA", "RGBX", "YCbCr"]:
                self.error = True
                raise ColorIMG
        ...
        try:
            ...
            for x in range(1, stacks + 1):
                img = merge_channels(
                    ...
                    empty_img,
                    ch_len,
                    slicescrollbar.value(),
                    color,
                    metaloc + x - 1,
                    pjt.findChildren(QRadioButton)[2].isChecked(),
                    bounds,
                    threshold,
                )
                ...
                # plotting
                if (
                    pjt.findChildren(QRadioButton)[1].isChecked() == False
                    or x == stacks
                ):
                    if pjt.findChildren(QRadioButton)[1].isChecked() == False:
                        axes = img_plot.fig.add_subplot(
                            int(np.ceil(np.sqrt(stacks))),
                            int(np.round_(np.sqrt(stacks), decimals=0)),
                            x,
                        )
                    else:
    ...
    def __call__(self, event):
        ...
        if (
            event.mouseevent.inaxes is not None
            ...
            x2, y2, z2 = proj3d.proj_transform(
                self.data[0][label_ind[0]],
                self.data[1][label_ind[0]],
                self.data[2][label_ind[0]],
            ...
            for i in range(np.shape(label_ind)[0]):
                ...
                #     matplotlib-annotating-a-3d-scatter-plot
                x2, y2, z2 = proj3d.proj_transform(
                    self.data[0][label_ind[i]],
                    self.data[1][label_ind[i]],
                    self.data[2][label_ind[i]],
                    self.main_plot.axes.get_proj(),
                )
                x3, y3 = self.main_plot.axes.transData.transform((x2, y2))
                # magic from https://stackoverflow.com/questions/10374930/
                ...
                if d < dmin:
                    dmin = d
                    imin = i
            self.main_plot.axes.scatter3D(
                self.data[0][label_ind[imin]],
                self.data[1][label_ind[imin]],
                self.data[2][label_ind[imin]],
                s=35,
                facecolor="none",
                edgecolor="gray",
                alpha=1,
            ...
            self.buildImageViewer(
                self.labels, label, imin, self.color, self.feature_file, self.imageID
            )
