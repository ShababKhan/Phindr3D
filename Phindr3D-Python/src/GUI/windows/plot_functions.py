...
def treatment_bounds(data, bounds, id, treatmentcol):
    ...
    try:
        ...
        win = errorWindow(
            "Error TreatmentNorm",
            "intensityNormPerTreatment was set with error: {}".format(ex),
        )
        return False
...
def merge_channels(
    ...
    for ind, rgb_color in zip(range(scroller_value, scroller_value + ch_len), color):
        ...
        data["Channel_" + ch_num] = data["Channel_" + ch_num].str.replace(
            r"\\", "/", regex=True
        )
        cur_img = np.array(
            PIL.Image.open(data["Channel_" + ch_num].iloc[meta_loc + scroller_value])
        )
        # medianfilter for slice or MIP projection
        if box == False:
            cur_img = medianBlur(cur_img, 3)
        # bounds from metadata functions compute
        cur_img = (cur_img - bounds[0][int(ch_num) - 1]) / (
            bounds[1][int(ch_num) - 1] - bounds[0][int(ch_num) - 1]
        )
    ...
    max_rng = [
        np.max(rgb_img[:, :, i]) if np.max(rgb_img[:, :, i]) > 0 else 1
        for i in range(3)
    ]
...
def result_plot(self, X, projection, plot, new_plot):
    ...
    colors = plt.cm.get_cmap("gist_ncar")(
        range(0, 255, floor(255 / len(np.unique(self.labels))))
    )
...
def legend_format(self, plot, colors, new_plot):
    ...
    text = ""
    handle = [
        matplotlib.patches.Patch(color=colour, label=label)
    ...
    if len(text) > 10:
        ...
        self.main_plot.axes.legend(
            handle, labels, bbox_to_anchor=bbox, ncol=cols, loc="center right"
        )
    else:
        self.main_plot.axes.legend(
            handle,
    ...
    if new_plot:
        self.original_xlim = [
            self.plots[-1].axes.get_xlim3d()[0],
            self.plots[-1].axes.get_xlim3d()[1],
        ]
        self.original_ylim = [
            self.plots[-1].axes.get_ylim3d()[0],
            self.plots[-1].axes.get_ylim3d()[1],
        ]
        self.original_zlim = [
            self.plots[-1].axes.get_zlim3d()[0],
...
def import_file(self, map_dropdown, colordropdown, twod, threed):
    ...
    filename = QFileDialog.getOpenFileName(
        self, "Open Plot Data JSON File", "", "JSON file (*.json)"
    )[0]
    if filename != "":
                    ...
                    if os.path.exists(data.get("feature_filename")):
                        ...
                        if len(plot_coord) == 3:
                            ...
                            self.loadFeaturefile(
                                colordropdown,
                                map_dropdown.currentText(),
                                False,
                                data.get("feature_filename"),
                            )
                        else:
                            errorWindow(
                                "Import Plot Data Error",
