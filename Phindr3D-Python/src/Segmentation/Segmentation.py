...
class Segmentation:
    ...
    def getCurrentIMs(self):
        ...
        if self.allIDs == []:
            ...
            return tf.imread(self.focusIms[self.allIDs[self.IDidx]]), tf.imread(
                self.labelIms[self.allIDs[self.IDidx]]
            )
    ...
    def RunSegmentation(self, mdata):
        ...
        try:
            for id in mdata.images:  # for each well:
                ...
                ll = [
                    getFocusplanesPerObjectMod(L == label, focusIndex)
                    for label in uLabels
                    if np.sum(L == label)
                    <= (L.size * self.settings["max_img_fraction"])
                ]
                ll = np.array(ll)
                ...
                filenameParts = [
                    f"{param[0]}{otherparams[param]}" for param in otherparams
                ]
                ...
                if len(ll) > 0:
                    ...
                    for iObjects in range(len(ll)):
                        for iPlanes in range(
                            int(ll[iObjects, 0]), int(ll[iObjects, 1] + 1)
                        ):
                            for chan in channels:
                                IM1 = tf.imread(
                                    imstack.stackLayers[iPlanes]
                                    .channels[chan]
                                    .channelpath
                                )
                                IM2 = IM1[fstruct[iObjects]]
                                tmpparts = filenameParts + [
                                    f"Z{iPlanes}",
                                    f"CH{chan}",
                                    f"ID{id}",
                                    f"OB{iObjects + 1}",
...
if __name__ == "__main__":
    ...
    print(
        f"Loading segmentation test images metadata success? ... {mdata.metadataLoadSuccess}"
    )
    ...
    for c, e in zip(
        os.listdir(segtest.labDir),
        os.listdir("testdata\\segmentation_tests\\expect_results\\LabelledImages"),
    ):
        check = os.path.abspath(segtest.labDir + "\\" + c)
        expect = os.path.abspath(
            "testdata\\segmentation_tests\\expect_results\\LabelledImages\\" + e
        )
    ...
    for c, e in zip(
        os.listdir(segtest.segDir),
        os.listdir("testdata\\segmentation_tests\\expect_results\\SegmentedImages"),
    ):
        check = os.path.abspath(segtest.segDir + "\\" + c)
        expect = os.path.abspath(
            "testdata\\segmentation_tests\\expect_results\\SegmentedImages\\" + e
        )
        same = (tf.imread(check) == tf.imread(expect)).all()
        if not same:
            break
