...
def smoothImage(IM, cutOff):
    ...
    if cutOff > 0.99:
        ...
        cutOff <= 0
    ):  # in matlab code is <. changed to <= because min value it is reset to is 0.1
        cutOff = 0.01
    m /= 2
...
def imfill(img):
    ...
    flooded = seg.flood_fill(
        tmp, (0, 0), 1, in_place=True
    )  # default is already full connectivity including diagonals as desired.
...
def getfsimage_roberts(imstack, segchannel):
    ...
    for z in zVals:
        IM = tf.imread(stackLayers[z].channels[segchannel].channelpath).astype(
            np.float64
        )
        edge = roberts(IM)
        mask = edge > maxEdge
...
def getfsimage_multichannel(imstack):
    ...
    for z in zVals:
        for i, c in enumerate(chans):
            if i == 0:
                IM = tf.imread(stackLayers[z].channels[c].channelpath).astype(
                    np.float64
                )
                ...
                IM = np.maximum(
                    IM,
                    tf.imread(stackLayers[z].channels[c].channelpath).astype(
                        np.float64
                    ),
                )
        imtmp = stdfilt(IM, kernel_size=5)
...
def getSegmentedOverlayImage(final_im, pdict):
    ...
    IM2 = cv.morphologyEx(
        final_im_downsampled.astype("uint16"), cv.MORPH_TOPHAT, SE
    ).astype("float64")
    ...
    IM6 = cv.morphologyEx(
        IM6, cv.MORPH_CLOSE, np.ones((3, 3))
    )  # this operation is iterated 3 times in matlab script. No reason to do this because morphological closing is idempotent.
    IM6 = imfill(IM6)
    IM6 = bwareaopen(
        IM6, 20
    )  # open sets of connected components with less than 20 members
    # im6 is binary image

    IM7 = ndimage.distance_transform_edt(
        IM6
    )  # matlab bwdist gives euclidean distance transform from non-zero elements. use on binary inverse of IM6 so distance transform from zero-elements. ndimage.distance_transform_edt is already distance from zero elements
    # im7 is float image
    ...
    if np.sum(areas) != 0:
        ...
        ii = (
            i2 * i3 * i4
        ) == 0  # ii is True at the indices of all the labels that we want to discard
...
def getFocusplanesPerObjectMod(labelImage, fIndex, numZ=None):
    ...
    try:
        ...
        bins = np.append(
            np.linspace(0.5, numZ - 0.5, num=int(numZ), endpoint=True), [numZ - 0.5]
        )  # this weird bin format gives a repeated final bin edge value, replicates histc function bin behaviour from matlab.
        n = np.histogram(ii, bins=bins)[0]  # 1d histogram array.
        n = n / np.sum(n)
        rndPoint = 1 / numZ
        p = (
            np.argwhere(n > rndPoint) + 1
        )  # find returns linear indices, so does argwhere, except starting at 0. for matlab , indices corresponds 1-1 to z-planes. need +1 to get proper z-planes values.
        if p.size == 0:
            minN = 1
            maxN = numZ
        else:
