...
class TrainingFunctions:
    ...
    def partition_data(mv_data, grps, select_grps):
        ...
        test_ind = np.delete(
            np.linspace(
                0, np.shape(mv_data)[0] - 1, num=np.shape(mv_data)[0], dtype=int
            ),
            train_ind,
            axis=0,
        )
    ...
    def selectClassesDF(self, featureDF, platemap):
        ...
        try:
            ...
            X_train, y_train, X_test, y_test = self.partition_data(
                mv, lbls, select_grps
            )
            ...
            for col in class_colnames:
                class_tbl[col + "_percentage"] = (
                    100 * class_tbl[col] / class_tbl[class_colnames].sum(axis=1)
                )
