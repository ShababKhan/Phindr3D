...
class VoxelBase:
    ...
    def getPixelBins(self, x, metadata, numBins, random_state=None):
        ...
        return VoxelFunctions.getPixelBins(
            x, metadata, numBins, random_state=random_state
        )
    ...
    def getTileProfiles(
        ...
        numTilesXY = int(
            ...
            / (theTileInfo.tileX * theTileInfo.tileY)
        )
        ...
        if intensityNormPerTreatment:
            ...
            try:
                grpVal = allTreatmentTypes.index(
                    imageObject.GetTreatment(treatmentColumnName)
                )
        ...
        if computeTAS:
            categoricalImage = np.zeros(
                (theTileInfo.croppedX, theTileInfo.croppedY, theTileInfo.croppedZ)
            )
        ...
        tmpData = np.zeros(
            (numTilesXY, int(theTileInfo.tileX * theTileInfo.tileY * theTileInfo.tileZ))
        )
        for iImages, zslice in enumerate(slices):
            ...
            for jChan in range(numChannels):
                ...
                try:
                    if intensityNormPerTreatment:
                        croppedIM[:, :, jChan] = dfunc.rescaleIntensity(
                            IM,
                        ...
                        croppedIM[:, :, jChan] = dfunc.rescaleIntensity(
                            IM, low=lowerbound[jChan], high=upperbound[jChan]
                        )
            ...
            x = np.reshape(
                croppedIM, (theTileInfo.croppedX * theTileInfo.croppedY, numChannels)
            )
            ...
            pixelCategory = (
                np.argmin(
                    np.add(
                        pixelBinCenterDifferences,
                        dfunc.mat_dot(x[fg, :], x[fg, :], axis=1),
                    ).T
                    - 2 * (x[fg, :] @ pixelBinCenters.T),
                    axis=1,
                )
                + 1
            )
            ...
            if computeTAS:
                categoricalImage[:, :, iImages] = np.reshape(
                    x, theTileInfo.croppedX, theTileInfo.croppedY
                )
            ...
            if sliceCounter == theTileInfo.tileZ:
                ...
                fgSuperVoxel[startVal:endVal] = (
                    np.sum(tmpData != 0, axis=1) / tmpData.shape[1]
                ) >= superVoxelThresholdTuningFactor
                ...
                tmpData = np.zeros(
                    (
                        numTilesXY,
                        theTileInfo.tileX * theTileInfo.tileY * theTileInfo.tileZ,
                    )
                )
            else:
                tmpData[:, startCol:endCol] = dfunc.im2col(
                    np.reshape(x, (theTileInfo.croppedX, theTileInfo.croppedY)),
                    (theTileInfo.tileX, theTileInfo.tileY),
                ).T
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            superVoxelProfile = np.divide(
                superVoxelProfile, np.array([np.sum(superVoxelProfile, axis=1)]).T
            )
            superVoxelProfile[np.isnan(superVoxelProfile)] = 0
    ...
    def getMegaVoxelProfile(
        ...
        temp1 = np.array(
            [dfunc.mat_dot(superVoxelBinCenters, superVoxelBinCenters, axis=1)]
        ).T
        temp2 = dfunc.mat_dot(
            tileProfile[fgSuperVoxel], tileProfile[fgSuperVoxel], axis=1
        )
        a = np.add(temp1, temp2).T - 2 * (
            tileProfile[fgSuperVoxel] @ superVoxelBinCenters.T
        )
        ...
        if analysis:
            ...
            for i in range(sv_image.shape[0]):
                ...
                try:
                    texture_features[0, :] = mt.features.haralick(
                        sv_image[i, :, :],
                        distance=1,
                        ignore_zeros=True,
                        return_mean=True,
                    ...
                    return errorVal
                try:
                    texture_features[1, :] = mt.features.haralick(
                        sv_image[i, :, :],
                        distance=2,
                        ignore_zeros=True,
                        return_mean=True,
                ...
                texture_features = texture_features[
                    ~np.isnan(texture_features).any(axis=1), :
                ]
            ...
            # end for
            total_mean_textures = total_mean_textures[
                ~np.isnan(total_mean_textures).any(axis=1), :
            ]
            ...
            try:
                if texture_features.size == 0:
                    ...
                    print(
                        "Texture feature extraction failed. continuing with default phindr3D"
                    )
        ...
        x = np.concatenate(
            [
                np.zeros((tileInfo.superVoxelZAddStart, x.shape[1], x.shape[2])),
                x,
                np.zeros((tileInfo.superVoxelZAddEnd, x.shape[1], x.shape[2])),
            ],
            axis=0,
        )  # new (z, x, y) shape
        ...
        x = np.concatenate(
            [
                np.zeros((x.shape[0], tileInfo.superVoxelXAddStart, x.shape[2])),
                x,
                np.zeros((x.shape[0], tileInfo.superVoxelXAddEnd, x.shape[2])),
            ],
            axis=1,
        )  # new (z, x, y) shape
        # pad third dimension
        x = np.concatenate(
            [
                np.zeros((x.shape[0], x.shape[1], tileInfo.superVoxelYAddStart)),
                x,
                np.zeros((x.shape[0], x.shape[1], tileInfo.superVoxelYAddEnd)),
            ],
            axis=2,
        )  # for new (z, x, y) shape
        x = x.astype(np.uint8)
        ...
        tileInfo.numMegaVoxelsXY = int(
            x.shape[1]
            * x.shape[2]
            / (tileInfo.megaVoxelTileY * tileInfo.megaVoxelTileX)
        )  # for new shape
        tileInfo.numMegaVoxels = int(
            (tileInfo.numMegaVoxelsXY * x.shape[0]) / tileInfo.megaVoxelTileZ
        )
        sliceCounter = 0
        startVal = 0
        endVal = tileInfo.numMegaVoxelsXY
        try:
            megaVoxelProfile = np.zeros(
                (tileInfo.numMegaVoxels, self.numSuperVoxelBins + 1)
            )
        ...
        for iSuperVoxelImagesZ in range(0, x.shape[0]):
            ...
            tmpData[:, startCol:endCol] = dfunc.im2col(
                x[iSuperVoxelImagesZ, :, :],
                (tileInfo.megaVoxelTileX, tileInfo.megaVoxelTileY),
            ).T
            ...
            if sliceCounter == tileInfo.megaVoxelTileZ:
                fgMegaVoxel[startVal:endVal] = (
                    np.sum(tmpData != 0, axis=1) / tmpData.shape[1]
                ) >= training.megaVoxelThresholdTuningFactor
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            megaVoxelProfile = np.divide(
                megaVoxelProfile, np.array([np.sum(megaVoxelProfile, axis=1)]).T
            )
            megaVoxelProfile[np.isnan(megaVoxelProfile)] = 0
    ...
    def getImageProfile(
        self, metadata, megaVoxelBinCenters, megaVoxelProfile, tileInfo, fgMegaVoxel
    ):
        ...
        tmp1 = np.array(
            [dfunc.mat_dot(megaVoxelBinCenters, megaVoxelBinCenters, axis=1)]
        ).T
        tmp2 = dfunc.mat_dot(
            megaVoxelProfile[fgMegaVoxel], megaVoxelProfile[fgMegaVoxel], axis=1
        )
        a = np.add(tmp1, tmp2).T - (
            2 * (megaVoxelProfile[fgMegaVoxel] @ megaVoxelBinCenters.T)
        )
        ...
        if showImage:
            mv_show = np.reshape(
                x,
                (
                    tileInfo.numMegaVoxelZ,
                    tileInfo.numMegaVoxelX,
                    tileInfo.numMegaVoxelY,
                ),
            )
            for i in range(mv_show.shape[0]):
                plt.figure()
                title = "Megavoxel image"
                plt.title(title)
        ...
        imageProfile = imageProfile / np.sum(
            imageProfile
        )  # normalize the image profile
